...
const Create = ({setTasks}) => {
  ...
  const navigate = useNavigate()
   ...
   const [title, setTitle] = useState('GYM Training')
   const [status, setStatus] = useState('Pending')
   const [category, setCategory] = useState('Work')
   const [priority, setPriority] = useState('Low')
   const userId = localStorage.getItem('id')

   const taskData = {date,title,status,category,priority,userId}
  ...
  return (
    <div className='absolute w-[80%] max-sm:w-full h-full left-[20%] max-sm:left-0 max-sm:top-0 bg-[#0a061d] light-mode-bg-main'>
       <div className='absolute w-full h-full bg-[#1a1631] rounded-l-[50px] max-sm:rounded-l-xl max-sm:rounded-r-xl flex justify-center light-mode-bg-secondary'>
           <p className='text-2xl font-bold text-white absolute top-5 mr-10 max-sm:mr-0 light-mode:text-black'>Add A New Task</p>
           
           <motion.div 
           variants={CreateVariants} initial='hidden' animate='visible'
           className='w-[80%] h-[550px] top-24 max-sm:top-16 max-sm:left-0 absolute'>
              ...
              <input onChange={(e)=>setTitle(e.target.value)} type="text" className='w-[650px] light-mode-bg-third max-sm:w-[345px] h-10 rounded-lg bg-[#27233e] text-white font-semibold pl-5 outline-none' placeholder={title} />
              </div>
              <div className='flex gap-8.5 items-center absolute top-28 left-[163px] max-sm:left-6'>
                ...
                <select onChange={(e)=>setPriority(e.target.value)} className='w-40 light-mode-bg-third h-10 rounded-lg bg-[#27233e] text-white cursor-pointer pr-2 font-semibold outline-none pl-2'>    
              ...
              <div className='flex gap-4 items-center absolute top-28 max-sm:top-60 left-[537px] max-sm:left-6'>
                ...
                <select onChange={(e)=>setStatus(e.target.value)} className='w-52 light-mode-bg-third max-sm:w-40 max-sm:ml-5.5 h-10 rounded-lg bg-[#27233e] text-white cursor-pointer pr-2 font-semibold outline-none pl-2'>    
              ...
              <div className='flex gap-4 items-center absolute top-48 left-[512px] max-sm:left-6 max-sm:top-[300px]'>
                ...
                <select onChange={(e)=>setCategory(e.target.value)} className='w-52 light-mode-bg-third max-sm:w-40 max-sm:ml-0.5 h-10 rounded-lg bg-[#27233e] text-white cursor-pointer pr-2 font-semibold outline-none pl-2'>    
              ...
              <button onClick={handleSubmit} className='w-48 max-sm:w-56 h-10 max-sm:top-[440px] left-96 max-sm:left-22 rounded-xl bg-[#ffd88d] font-semibold cursor-pointer hover:bg-[#e5bf7a] hover:duration-200 mr-10 top-72 absolute light-mode-black-button'>Save</button>
